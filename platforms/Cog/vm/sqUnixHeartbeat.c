...
ioHighResClock(void)
{
  ...
  sqLong value = 0;
	...
	long long theTime = mach_absolute_time();
	value = theTime & 0xFFFFFFFF;
#else	
...
ioRelinquishProcessorForMicroseconds(int microSeconds)
{
    ...
    if (nextWakeupUsecs <= utcNow) {
        ...
        if (nextWakeupUsecs != 0) {
			sqLong much = (sqLong)nextWakeupUsecs - (sqLong)utcNow;
//			fprintf(stderr," <%i",much);
			if (much < -1000) {
//				fprintf(stderr,".");
				return;
			}
		...
		if (realTimeToWait < 1000) 
			realTimeToWait = 1000;
//		fprintf(stderr," S%i",realTimeToWait);
	}
